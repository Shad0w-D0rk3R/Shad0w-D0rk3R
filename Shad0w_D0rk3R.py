...
class Dorker(object):
    def __init__(self):
        ...
        self.domains = ['ac', 'ad', 'ae', 'af', 'ag', 'ai', 'al', 'am', 'an', 'ao',
                        ...
                        'pro', 'travel']
        try:
            self.Get_list = raw_input(self.r + '    [+]' + self.c + ' Enter Dork List File_Name: ' + self.y)
        except IOError:
            print self.r + '--------------------------------------------'
            print self.r + '    [' + self.y + '-' + self.r + '] ' + self.c + ' List Not Found in Directory!'
            sys.exit()
        self.q = Queue(self.concurrent * 8)
        for i in range(self.concurrent):
            self.t = threading.Thread(target=self.doWork3)
            self.t = threading.Thread(target=self.doWork2)
            self.t = threading.Thread(target=self.doWork1)
            ...
            with open(line, "r") as infile:
                for line1 in infile:
                    if line1.startswith("http://"):
                        line1 = line1.replace("http://", "http://")
                    elif line1.startswith("https://"):
                        line1 = line1.replace("https://", "https://")
                    else:
                        pass
                    x = '' + line1
                    parse = urlparse(x)
                        ...
                        for url in finder:
                            ...
                            with open("logs/logs.txt", 'a') as f:
                                if 'go.microsoft.com' in url:
                                    ...
                                    try:
                                        self.cur_execute("INSERT INTO dorker_db('url') VALUES(?)", ('' + url,))
        ...
        try:
            while True:
                ...
                for domain in self.domains:
                    next = 0
                    while next <= 500:
                        url = 'http://www.google.com/search?q=' + dork + ' site:' + domain + '&first=' + str(
                            next) + '&FORM=PORE'
                        sess = requests.session()
                        cnn = sess.get(url, timeout=5)
                        next = next + 10
        ...
        try:
            while True:
                ...
                for domain in self.domains:
                    next = 0
                    while next <= 500:
                        url = 'http://www.ask.com/web?q=' + dork + ' site:' + domain + '&first=' + str(
                            next) + '&FORM=PORE'
                        sess = requests.session()
                        cnn = sess.get(url, timeout=5)
                        next = next + 10
        ...
        try:
            while True:
                ...
                for domain in self.domains:
                    next = 0
                    while next <= 500:
                        url = 'http://nova.rambler.ru/search?query=' + dork + ' site:' + domain + '&first=' + str(
                            next) + '&FORM=PORE'
                        sess = requests.session()
                        cnn = sess.get(url, timeout=5)
                        next = next + 10
                        ...
                        for url in finder:
                            ...
                            if url.startswith('www.'):
                                ...
                                pass
                            with open("logs/logs3.txt", 'a') as f:
                                if 'go.microsoft.com' in url:
                                    pass
                                elif '.wordpress.' in url:
                                    pass
...
class Main():
    ...
    def print_logo(self):
        ...
        cprint(figlet_format('Shad0w_D0rk3R', font='small'),
